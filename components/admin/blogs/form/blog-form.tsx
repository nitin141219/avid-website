...
export function BlogForm({ blogId, blogData, blogsData }: BlogFormProps) {
  ...
  useEffect(() => {
    ...
    reset({
      slug: blogData.slug,
      title_en: blogData.title_en || blogData.title || "",
      title_de: blogData.title_de || "",
      title_fr: blogData.title_fr || "",
      title_es: blogData.title_es || "",
      sub_title_en: blogData.sub_title_en || blogData.sub_title || "",
      sub_title_de: blogData.sub_title_de || "",
      sub_title_fr: blogData.sub_title_fr || "",
      sub_title_es: blogData.sub_title_es || "",
      content_en: blogData.content_en || blogData.content || "",
      ...
      related_blogs: blogData.related_blogs?.map((blog: any) => ({
        value: blog._id,
        label: blog.title,
      })),
    });
  ...
  const onSubmit = async (values: BlogFormValues) => {
    ...
    const relatedIds = values.related_blogs.map((blog: any) => blog.value);
    ...
    formData.append("related_blogs", JSON.stringify(relatedIds));
    ...
    try {
      if (blogId) {
        await BLOGS_SERVICES.updateBlog(blogId, formData);
  ...
  const allBlogsList = useMemo(() => {
    return blogsData
      ?.filter((blog: any) => blog._id !== blogId)
      ?.map((blog: any) => ({
        value: blog._id,
        label: blog.title,
      }));
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="gap-5 space-y-5 grid grid-cols-3">
        ...
        <div>
          <Label className="mb-2 font-medium md:text-sm">Related Blogs</Label>
          <Controller
            name="related_blogs"
            control={control}
            render={({ field }) => (
              <CustomSelect
                ...
                {...field}
                isMulti
                options={allBlogsList}
              />
            )}
          />
          {errors.related_blogs && (
            <p className="mt-1 text-red-500 text-sm">{errors.related_blogs.message}</p>
          )}
        </div>
        {/* Active Status */}
